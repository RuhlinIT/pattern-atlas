...
class TaskAdapter {
    public static function fromExternal(ExternalTask $task): Task {
    ...
    public static function fromExternalList(array $tasks): array {
        return array_map(fn($task) => self::fromExternal($task), $tasks);
    }
}
...
class TaskService {
    ...
    public function __construct(TaskApiService $api) {
        $this->api = $api;
    }
...
$service = new TaskService(new TaskApiService());
